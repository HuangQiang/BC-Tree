...
// -----------------------------------------------------------------------------
template<class DType>
void k_truth(                       // find k ground truth for a query
    int   n,                            // number of data  objects
    int   d,                            // dimension of space
    const DType *data,                  // data  objects
    const float *query,                 // query objects
    MinK_List *list)                    // k truth in the list
{
    list->reset();
    for (int j = 0; j < n; ++j) {
        float dist = calc_p2h_dist<DType>(d, data+(u64)j*d, query);
    ...
    const float *query)                 // query objects
{
    ...
    // grouth truth
    MinK_List **list = new MinK_List*[qn];
    for (int i = 0; i < qn; ++i) list[i] = new MinK_List(MAXK);
    
    #pragma omp parallel num_threads(THREAD_NUM)
    {
        #pragma omp for
        for (int i = 0; i < qn; ++i) {
            k_truth(n, d, data, query+i*d, list[i]);
            // printf("i=%d\n", i);
        }
    }
    ...
    for (int i = 0; i < qn; ++i) {
        ...
        for (int j = 0; j < MAXK; ++j) {
            fprintf(fp, ",%d,%.9f", list[i]->ith_id(j), list[i]->ith_key(j));
        }
        fprintf(fp, "\n");
    }
    fclose(fp);
    for (int i = 0; i < qn; ++i) delete list[i];
    delete[] list;
