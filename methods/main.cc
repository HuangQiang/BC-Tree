...
void usage()                        // display the usage
{
    printf("\n"
        ...
        " -m     {integer}  #hash tables (FH, NH, Ortho_LSH, P2LSH)\n"
        " -l     {integer}  leaf size ratio (Ball_Tree, KD_Tree)\n"
        ...
        "\n"
        " 6  - NH_DCF (Nearest Hyperpalne Hash with QALSH)\n"
        "      Param: -alg 6 -n -qn -d -m -s -cf -dt -dn -ds -qs -ts -op\n"
        "\n"
        "-------------------------------------------------------------------\n"
...
void impl(                          // implementation interface
    ...
    int   leaf,                         // leaf size of ball-tree/kd-tree
    int   m,                            // #tables (FH,NH,Ortho_LSH)
    ...
    const char *path)                   // output path
{
    ...
    case 5:
        ...
        break;
    case 6:
        nh_dcf<DType>(n, qn, d, m, s, conf_name, data_name, "NH_DCF", path, 
            (const DType*) data, (const float*) query, (const Result*) R);
        break;
    default:
...
int main(int nargs, char **args)
{
    ...
    int   leaf = -1;                // leaf size of ball-tree/kd-tree
    int   m    = -1;                // #tables (FH,NH,Ortho_LSH)
    int   s    = -1;                // scale factor of dimension (s > 0) (FH,NH)
    ...
    while (cnt < nargs) {
        if (strcmp(args[cnt], "-alg") == 0) {
            ...
            float frac = atof(args[++cnt]); 
            leaf = (int) ceil(frac*n); assert(leaf > 1);
            printf("leaf      = %d\n", leaf);
        }
